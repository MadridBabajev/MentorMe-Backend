...
public class AccountController : ControllerBase
{
        ...
        int expiresInSeconds)
    {
        ...
        if ((registrationData.IsTutor && isTutorProfileExists) || (!registrationData.IsTutor && isStudentProfileExists))
        {
            ...
            return FormatErrorResponse($"User with email {registrationData.Email} is already registered with the given user type");
        }
        ...
        ECountry userCountry = registrationData.Country switch
        {
            "Estonia" => ECountry.Estonia,
            "Latvia" => ECountry.Latvia,
            "Lithuania" => ECountry.Lithuania,
            _ => ECountry.Estonia
        };
        ...
        if (registrationData.IsTutor)
        {
            var tutor = new Tutor {Id = appUser.Id, AppUserId = appUser.Id};
            _context.Tutors.Add(tutor);
        }
        else
        {
            var student = new Student {Id = appUser.Id, AppUserId = appUser.Id};
    ...
    public async Task<ActionResult<JWTResponse>> LogIn([FromBody] Login loginData, [FromQuery] int expiresInSeconds)
    {
        ...
        {
            _logger.LogWarning("WebApi login failed, email {} not found", loginData.Email);
            return FormatErrorResponse("No such user with the given user type was found");
        }
        ...
        [FromQuery] int expiresInSeconds)
    {
        ...
        {
            return FormatErrorResponse($"RefreshTokens collection is null or empty - {appUser.AppRefreshTokens?.Count}");
        }
